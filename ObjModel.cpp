...
/// 静的メンバ変数の実体
/// </summary>
const std::string ObjModel::baseDirectory = "Resources/";
...
ObjModel* ObjModel::CreateFromOBJ(const std::string& ObjModelname, bool smoothing)
{
	// メモリ確保
	ObjModel* instance = new ObjModel;
	instance->Initialize(ObjModelname, smoothing);
...
void ObjModel::Initialize(const std::string& ObjModelname, bool smoothing)
{
	// モデル読み込み
	LoadModel(ObjModelname, smoothing);
...
void ObjModel::LoadModel(const std::string& ObjModelname, bool smoothing)
{
	const string filename = ObjModelname + ".obj";
	const string directoryPath = baseDirectory + ObjModelname + "/";
